...
![FareWeave – selbst gehosteter Reisevergleich ohne KI](docs/assets/readme-hero.webp)
...
In FareWeave 0.0.1 werden zuerst **Skiplagged, Ryanair, Vueling und easyJet** abgefragt. Reichen die Ergebnisse nicht aus, folgen **Transavia, Norwegian, Air France/KLM und Wizz Air**.
...
Google Flights ist zusätzlich als manueller Gegencheck eingebaut. Es handelt sich in FareWeave 0.0.1 dabei **nicht um einen eigenen automatischen Google-Flights-Provider**, sondern um einen passenden Suchlink für die betreffende Strecke und die gewählten Reisedaten.
...
Zusätzlich wurde Stay22 separat in FareWeave integriert. Darüber werden aktuell **Expedia und Hotels.com** als weitere Unterkunftsquellen abgefragt. Ein eigener `STAY22_API_KEY` kann optional hinterlegt werden.
...
Die Oberfläche ist standardmäßig ausschließlich lokal unter <http://127.0.0.1:8791> erreichbar. Der Healthcheck liegt unter <http://127.0.0.1:8791/api/health>.

Wer FareWeave bewusst im LAN veröffentlichen möchte, kann in `.env` setzen:
...
Der Build leitet die interne Versionsnummer aus dem Tag ab und prüft die von FareWeave verwendeten Hotel-, Flug- und Ground-CLI-Verträge. Ein inkompatibles trvl-Release soll dadurch nicht stillschweigend übernommen werden.
...
- [Stay22](https://www.stay22.com/) – zusätzliche Unterkunftssuche; in FareWeave aktuell auf Expedia und Hotels.com begrenzt.

Weitere Abhängigkeiten und die jeweils geltenden Lizenzbedingungen stehen in [THIRD_PARTY.md](THIRD_PARTY.md).

FareWeave ist unabhängig von Deutsche Bahn, BetterBahn, Flix, trvl, Transitous, Stay22 und den übrigen abgefragten oder verlinkten Reiseanbietern. Marken und Namen gehören ihren jeweiligen Rechteinhabern.
